	   ...
	   if (myN->subnuc)
	     {
	       ...
	       Collide(sample,sampleB,myN,myN,b,lambda,&Np,&Nc,ncoll);	  
	       if (Np>0)
		 {
	   ...
	   else
	     {
	       ...
	       //if (Np>0) //select
	       if ((Np>106)&&(Np<158)) //PbPb 30-40 percent class?
		 {
		   printf("ev=%i np=%i\n",ev,Np);
		   int Nc=Ncoll(sample,sampleB,myN,myN,ncoll,b,sigma);
		   recenter(ncoll,2*Nc);
		   Nclist[ev]=Nc;
		   Nplist[ev]=Np;
		   char outdir[255];
